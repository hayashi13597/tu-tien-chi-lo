...
describe('GetCultivationStateUseCase', () => {
  it('rejects an unknown user with CHARACTER_NOT_FOUND', async () => {
    const useCase = new GetCultivationStateUseCase(new InMemoryCharacterRepository(), new StaticRealmConfigSource(), new InMemoryOwnedCongPhapRepository());
  ...
  it('reports canBreakthrough=false and isMaxStage=false for a fresh Phàm Nhân - Sơ character', async () => {
    ...
    const result = await new GetCultivationStateUseCase(characters, new StaticRealmConfigSource(), new InMemoryOwnedCongPhapRepository()).execute('user-1');
...
describe('GetCultivationStateUseCase — out-of-range clamp', () => {
  const BASE_ATTRS = { baseKhiHuyet: 40, baseChanNguyen: 30, baseCongVatLy: 6, baseCongPhep: 6, basePhongThu: 4, baseTocDo: 2 };
  const smallConfig = new RealmConfigSet([
    { name: 'A', subStages: [
      { name: 'A0', linhKhiRequired: 100, cultivationRate: 1, baseSuccessRate: 90, pityIncrement: 10, maxSuccessRate: 95, punishmentSeconds: 300, ...BASE_ATTRS },
      { name: 'A1', linhKhiRequired: 200, cultivationRate: 1.2, baseSuccessRate: 88, pityIncrement: 10, maxSuccessRate: 95, punishmentSeconds: 400, ...BASE_ATTRS },
    ] },
  ]);

  function outOfRangeCharacter(): CharacterRecord {
    return {
      id: 'c1', userId: 'user-1', realmMajor: 3, realmSub: 9, linhKhi: 0, linhThach: 0,
  ...
  it('clamps an out-of-range character to the nearest valid stage and persists it', async () => {
    ...
    const useCase = new GetCultivationStateUseCase(characters, new StaticRealmConfigSource(smallConfig), new InMemoryOwnedCongPhapRepository());
